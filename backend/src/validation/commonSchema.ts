...
export const pageSchema = z.int().min(1);
export const countPerPageSchema = z.int().min(1).max(100);
...
export const channelMessageSchema = z.string().min(1, "Channel message is required").max(1024, "Channel message cannot exceed 1024 characters");
export const indexSchema = z.int("Index must be an integer").min(0, "Index must be at least 0");
...
export const voteSchema = z.int("Vote must be an integer").min(0, "Vote must be 0 or 1").max(1, "Vote must be 0 or 1");
